...
MiniHTTPD::run()
{
	...
	while (1) {
		...
		if (strstr(line, ".js") != NULL)
		{
			fprintf( fout, "Content-type: text/javascript\r\n\r\n");
		} 
		else if (strstr(line, ".css") != NULL)
		{
			fprintf( fout, "Content-type: text/css\r\n\r\n");
		}
		else
		{
		...
		while (*p != 0 && *p !=' ') p++;//get/post etc skipped here, for future reference
